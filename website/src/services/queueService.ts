...
class UpdateQueue {
  ...
  enqueue(item: Omit<QueueItem, "timestamp">) {
    ...
    const existingIndex = this.queue.findIndex(
      (i) =>
        i.type === item.type && i.id === item.id && i.action === item.action
    );
  ...
  private async processRestaurantItem(item: QueueItem) {
    ...
    const { useRestaurantStore } = await import("../store/restaurantStore");

    useRestaurantStore.getState().decrementPendingChanges();
  }
